...
Branch `feat/new-model-integrations` exists (Plan A, Task 1). Task 2 below needs a HuggingFace **write** token for the `sensein` org — an operator step, not something the implementing agent can do with a read-scoped session token.
