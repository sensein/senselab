"""Join the two replay sides and report conformance and readings per family, with denominators.

``specs/20260921-gates-in-verdict/corpus-replay-report.py``, with one addition: it also counts the
recordings whose measure readings differ between the sides, which is the wider surface this change
could have moved.
...
def main() -> None:
    ...
    reading_moves: list[dict[str, Any]] = []
    ...
    for stem in stems:
        ...
        if before.get("readings") != after.get("readings"):
            counter["readings_moved"] += 1
            was, now = before.get("readings") or {}, after.get("readings") or {}
            differing = sorted(key for key in set(was) | set(now) if was.get(key) != now.get(key))
            reading_moves.append({"stem": stem, "family": family, "readings": differing})
    ...
    print("| family | n | before True/False/UND | after True/False/UND | moved | readings moved |")
    print("| --- | --- | --- | --- | --- | --- |")
    ...
    for family in sorted(per_family):
        ...
        print(
            f"| `{family}` | {counter['n']} | {shown_before} | {shown_after} | "
            f"{counter['moved']} | {counter['readings_moved']} |"
        )
    ...
    print(
        f"| **all** | **{totals['n']}** | **{all_before}** | **{all_after}** | "
        f"**{totals['moved']}** | **{totals['readings_moved']}** |"
    )
    ...
    print(f"\nrecordings whose measure readings differ: {len(reading_moves)}")
    by_reading: collections.Counter[str] = collections.Counter()
    for row in reading_moves:
        by_reading[f"{row['family']}: {', '.join(row['readings'])}"] += 1
    for label, count in by_reading.most_common(20):
        print(f"  {count:>6}  {label}")
