...
- *Caveat — band-recalibration arm void:* the test also compared scoring under the build-grid band vs a detect-grid-recalibrated band, but on clean single-speaker enrollment both resolve to the literature fallback `(0.30, 0.70)` (no minority cluster → no empirical band), so they were identical and the recalibration question is unanswered here. The band-mismatch concern only manifests with *contaminated* enrollment (where the band goes empirical, e.g. a real subject's `(0.5, 0.9)` ECAPA band); testing recalibration needs a multi-cluster build set.
