...
- [X] T047a Diarization-overlap corroborator. `compare_recording_to_profile(diar_overlap_by_window=…)` raises the consensus other-voice uncertainty to `max(profile_value, DIAR_OVERLAP_OTHER_VOICE_FLOOR)` (default 1.0, configurable) on windows where diarization sees 2+ distinct speakers — reference-free, so it corroborates even for a low-confidence profile and exactly where the profile-distance is unreliable (mixed-speaker windows). `analyze_audio._diar_overlap_for_windows` builds the mask from the pass diarization (None when no diar ran → no change; opt-in/additive). Flows through the option-C identity voter. Model-free unit test added; ruff/mypy clean. (commit `aa734c3c`)
