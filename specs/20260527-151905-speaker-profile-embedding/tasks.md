...
- [X] T042 [P] Promote one shared cosine helper (`embeddings.cos_sim`/`cos_dist`, single degenerate-input contract, `cos_dist(clip=)` covers identity `[0,1]` vs profile unclipped) and replace the three copies (`compare._cosine_distance`, `clustering._cos_sim`, `identity._cosine_similarity`/`_cos_dist`). **Verified**: numerically equivalent (compare/clustering bit-identical; identity 1-ULP, neutral under within-run SC-006), ruff+mypy clean, io/cache unit tests pass. *GPU SC-006 (T029) + compare_test confirmation pending on a compute node.* (commit `4d413184`)
