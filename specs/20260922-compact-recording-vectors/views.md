...
> **The release values recorded below are the pre-2026-09-24 vocabulary**, which is what the
> runs they record used. `releasable` is now `release_with_redaction`; `nothing_to_redact`
> split, with three of its grounds becoming `release_without_redaction` and `NO_TRANSCRIPT`
> becoming a `not_assessed` ground.
