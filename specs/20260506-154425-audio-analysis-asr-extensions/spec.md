...
- **SC-011**: After a successful run, re-invoking the script with `--no-align-asr` removed (or with a different aligner backend) reuses the ASR cache (no ASR call) and re-runs only the alignment step. Verifiable by per-step `cache: hit/miss` reporting on the ASR vs alignment outputs.
