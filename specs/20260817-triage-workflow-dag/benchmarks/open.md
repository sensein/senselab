...
| **Whether routing's empty execution set should carry a `flag` verdict** | verdict.md's "acoustically empty → discard" row, which is currently unreachable | `fold_file_verdict` tests any-`flag` before every-kind-absent, so routing's own empty-set `flag` decides first and an all-absent recording reads `flag`, not the `fail`/discard the fold's later branch would have given it. Either the fold orders the two deliberately, or routing records the empty set on its decisions without a `flag` verdict and the fold reads it off them. T8 owns the choice |
