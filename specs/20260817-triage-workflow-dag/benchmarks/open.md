...
| **Whole-corpus enrollment centroids are contaminated by non-lexical files** | enrolling over ALL of a subject's files pulls the ECAPA centroid toward cough/breath/DDK material (HardCough cosine -0.045 against its own subject's centroid); leave-one-out medians 0.51-0.64 straddle the guessed 0.5 cut, so target attribution lands at 53%. The fit owed: the cut, and an enrollment-source policy (lexical files only, or the runbook's exclusion list) |
