...
and its derivation names the carrier and the classifier measurements. **This costs REPORT.**
`report.py:_airway_labels` reads `verb == "label"` assertions marked against a span id, so an
AIRWAY span now renders as `airway span: unlabelled`. The fix is REPORT reading the span's own
`label` attribute, which is contract piece 7's territory and not this branch's file.
...
* **REPORT reading a span's `label` attribute** instead of a `label` assertion.
