...
out.append(
    "Every gate below applies identically to every task. `branch:` carries no per-family "
...
out.append(f"**{len(groups)} distinct settings groups over {sum(len(v) for v in groups.values())} families.**\n")
