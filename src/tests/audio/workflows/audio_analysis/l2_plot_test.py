...
def test_a_round_with_rows_gets_a_figure(tmp_path) -> None:  # noqa: ANN001
    ...
    path = build_round_timeline(tmp_path, round_index=0, axis_rows={"identity": _rows([0.1, 0.9])}, duration_s=1.0)
...
def test_each_round_writes_its_own_file(tmp_path) -> None:  # noqa: ANN001
    ...
    a = build_round_timeline(tmp_path, round_index=0, axis_rows={"identity": _rows([0.5])}, duration_s=0.5)
    b = build_round_timeline(tmp_path, round_index=1, axis_rows={"identity": _rows([0.2])}, duration_s=0.5)
...
def test_a_round_with_no_rows_writes_nothing(tmp_path) -> None:  # noqa: ANN001
    ...
    assert build_round_timeline(tmp_path, round_index=0, axis_rows={"identity": []}, duration_s=1.0) is None
...
def test_unmeasured_buckets_leave_gaps_rather_than_interpolating(tmp_path) -> None:  # noqa: ANN001
    ...
    path = build(tmp_path, round_index=0, axis_rows={"identity": _rows([0.4, None, 0.4])}, duration_s=1.5)
    assert path is not None and path.exists()
