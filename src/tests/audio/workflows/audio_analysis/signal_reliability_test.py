...
def _harvest(perturbation: str, values: dict[str, float]) -> object:
...
def test_a_signal_that_answers_the_same_under_perturbation_is_reliable() -> None:
    ...
    stab = signal_stability(
        {"raw": _harvest("raw", {"a": 0.2}), "enhanced": _harvest("enhanced", {"a": 0.2})},
        axis="speaker",
    )
...
def test_a_signal_that_flips_under_perturbation_is_not() -> None:
    ...
    stab = signal_stability(
        {"raw": _harvest("raw", {"a": 0.0}), "enhanced": _harvest("enhanced", {"a": 1.0})},
        axis="speaker",
    )
...
def test_a_single_pass_yields_no_reliability_claim() -> None:
    ...
    """
    stab = signal_stability({"raw": _harvest("raw", {"a": 0.3})}, axis="speaker")
    assert stab == {}
...
def test_signals_are_scored_independently() -> None:
    ...
    stab = signal_stability(
        {
            "raw": _harvest("raw", {"steady": 0.2, "flipper": 0.0}),
            "enhanced": _harvest("enhanced", {"steady": 0.2, "flipper": 1.0}),
        },
        axis="speaker",
    )
