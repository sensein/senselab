...
def _authored_by(store: ProvStore, entity: Entity, node: str) -> bool:
    """Whether one node's activity generated this entity, rather than the fixture or PREPROCESS.
    ...
    Returns:
        Whether it was generated by that node.
    """
    activity_id = store.generated_by(entity.id)
    return activity_id is not None and store.get_activity(activity_id).node == node


class TestEveryLocatedFindingReachesTheStoreWithItsEvidence:
    """An extent beside a span is coincidence; the edge is what a reader can follow back."""

    def test_no_assertion_or_measurement_over_an_extent_stands_unsourced(
        self, store: ProvStore, airway_config: TriageConfig, tmp_path: Path
    ) -> None:
        """Over a run that emits every in-family kind, each located finding names what it was read off."""
        ...
        assert located, "the fixture has to emit located findings for this to test anything"
        unsourced = [
            entity.attributes.get("name") or entity.attributes.get("deviation_type")
            for entity in located
            if not store.derived_from(entity.id)
        ]
        assert unsourced == []
