...
def _seed_airway_store(  # noqa: C901 — one independent block per derivative, as PREPROCESS has
    ...
    *,
    ...
    bracketed_words: list[tuple[str, tuple[float, float]]] | None = None,
    ...
    Args:
        ...
        bracketed_words: ``[(bracketed, (start, end)), ...]`` bracketed consensus words, indexed after ``words``.
    ...
    Returns:
        The ids of what was written, keyed ``plain``/``spans``/``words``/``bracketed``/``span_hear``/
        ``yamnet``/``silence``.
    """
    ...
    ids: dict[str, Any] = {"spans": [], "words": [], "bracketed": [], "span_hear": [], "yamnet": []}
    ...
    lexical = list(words if words is not None else [])
    for index, (text, extent) in enumerate(lexical):
        ids["words"].append(_write("word", extent, word_attributes(text, extent, index=index)))
    for offset, (text, extent) in enumerate(bracketed_words if bracketed_words is not None else []):
        ids["bracketed"].append(_write("word", extent, word_attributes(text, extent, index=len(lexical) + offset)))
...
class TestItReadsPreprocessSpanHear:
    ...
    def test_a_span_carrying_only_bracketed_words_stays_eligible(
        self, store: ProvStore, airway_config: TriageConfig, tmp_path: Path
    ) -> None:
        """A bracketed word — [COUGH] — is exactly what this branch is looking for, not a transcript."""
        _seed_airway_store(
            store,
            tmp_path,
            spans=[(1.0, 1.3, 30.0)],
            hear_by_span=[["Cough"]],
            bracketed_words=[("[COUGH]", (1.0, 1.2))],
        )
        airway(store, "plain", airway_config, run_dir=tmp_path)
...
class TestLexicalContamination:
    """The interval spans the gaps; a bracketed word is not lexical and a word outside it is not contamination."""
    ...
    def test_a_word_in_the_gap_between_labelled_spans_flags_by_id_only(
        ...
        ids = _seed_airway_store(
            ...
            bracketed_words=[("[COUGH]", (1.85, 1.95))],
        )
        result = airway(store, "plain", airway_config, run_dir=tmp_path)
