...
def test_audio_data_augmentation():
    ...
    assert torch.equal(mono_audio[0].audio_data, -1*mono_inverted[0].audio_data), "Audio should have been inverted by the augmentation"
    assert torch.equal(stereo_audio[0].audio_data, -1*stereo_inverted[0].audio_data), "Audio should have been inverted by the augmentation and not affected by stereo audio"
    
    batched_audio = AudioDataset.generate_dataset_from_audio_data(
        [stereo_audio[0].audio_data[0], stereo_audio[0].audio_data[1]],
        stereo_audio[0].sampling_rate
    ).create_split_for_pydra_task(True, 2)
    batch_inverted = augment_audio_dataset(batched_audio[0], apply_augmentation, batched=True)
    assert (
        torch.equal(batched_audio[0][0].audio_data, -1*batch_inverted[0].audio_data)
        and torch.equal(batched_audio[0][1].audio_data, -1*batch_inverted[1].audio_data)
    )
