...
def test_audio_length_positive_check(mono_audio_sample: Audio) -> None:
    ...
    results = audio_length_positive_check([mono_audio_sample], [mono_audio_sample])
    assert mono_audio_sample in results["passed"], "Valid audio should pass length check."
    assert len(results["exclude"]) == 0, "Valid audio should not be excluded."

    # Check empty audio (should be excluded)
    results = audio_length_positive_check([empty_audio], [empty_audio])
    assert empty_audio in results["exclude"], "Empty audio should be excluded."
    assert len(results["passed"]) == 0, "Empty audio should not pass."
...
def test_audio_intensity_positive_check(stereo_audio_sample: Audio) -> None:
    ...
    # Check valid audio (should pass)
    results = audio_intensity_positive_check([stereo_audio_sample], [stereo_audio_sample])
    assert stereo_audio_sample in results["passed"], "Valid audio should pass intensity check."
    assert len(results["exclude"]) == 0, "Valid audio should not be excluded."

    # Check silent audio (should be excluded)
    results = audio_intensity_positive_check([silent_audio], [silent_audio])
    assert silent_audio in results["exclude"], "Silent audio should be excluded."
    assert len(results["passed"]) == 0, "Silent audio should not pass."
