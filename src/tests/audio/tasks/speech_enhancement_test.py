...
if os.getenv("GITHUB_ACTIONS") != "true":
    ...
    def test_enhance_audios_stereo_audio(resampled_stereo_audio_sample: Audio, 
                                         speechbrain_model: SpeechBrainModel) -> None:
        """Test that enhancing stereo audios raises a ValueError."""
        with pytest.raises(ValueError, match="Audio waveform must be mono"):
            SpeechBrainEnhancer.enhance_audios_with_speechbrain(audios=[resampled_stereo_audio_sample], 
                                                                model=speechbrain_model)

    def test_enhance_audios(resampled_mono_audio_sample: Audio, 
                            speechbrain_model: SpeechBrainModel) -> None:
    ...
    def test_enhance_audios_with_speechbrain(resampled_mono_audio_sample: Audio, 
                                             speechbrain_model: SpeechBrainModel) -> None:
    ...
    def test_enhance_audios_incorrect_sampling_rate(mono_audio_sample: Audio, 
                                                    speechbrain_model: SpeechBrainModel) -> None:
        ...
        with pytest.raises(ValueError, match="Audio sampling rate 8000 does not match expected 16000"):
            SpeechBrainEnhancer.enhance_audios_with_speechbrain(audios=[mono_audio_sample], 
                                                                model=speechbrain_model)
