...
if os.getenv("GITHUB_ACTIONS") != "true":
    ...
    def test_enhance_audios_stereo_audio(resampled_stereo_audio_sample: Audio, hf_model: HFModel) -> None:
        """Test that enhancing stereo audios raises a ValueError."""
        with pytest.raises(ValueError, match="Audio waveform must be mono"):
            SpeechBrainEnhancer.enhance_audios_with_speechbrain(audios=[resampled_stereo_audio_sample], model=hf_model)

    def test_enhance_audios(resampled_mono_audio_sample: Audio, hf_model: HFModel) -> None:
        """Test enhancing audios."""
        enhanced_audios = enhance_audios(audios=[resampled_mono_audio_sample, 
    ...
    def test_enhance_audios_with_speechbrain(resampled_mono_audio_sample: Audio, hf_model: HFModel) -> None:
        """Test enhancing audios with SpeechBrain."""
        enhanced_audios = SpeechBrainEnhancer.enhance_audios_with_speechbrain(audios=[resampled_mono_audio_sample], 
                                                                            model=hf_model)
    ...
    def test_enhance_audios_incorrect_sampling_rate(mono_audio_sample: Audio, hf_model: HFModel) -> None:
        ...
        with pytest.raises(ValueError, match="Audio sampling rate 8000 does not match expected 16000"):
            SpeechBrainEnhancer.enhance_audios_with_speechbrain(audios=[mono_audio_sample], model=hf_model)
