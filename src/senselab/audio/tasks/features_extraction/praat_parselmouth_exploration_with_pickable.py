...
class _PickableSound:
    """A wrapper to the Parselmouth Sound object.

    This class allows you to store the data and sampling frequency from the
    parselmouth.Sound object and convert it back to a parselmouth.Sound object.
    This is useful since the parselmouth.Sound object is not pickable.
    """
    def __init__(self, sound: parselmouth.Sound) -> None:
        """Initialize the _PickableSound object.

        Args:
            sound (parselmouth.Sound): The parselmouth.Sound object to store.
        """
        # Store the data and sampling frequency from the parselmouth.Sound object
        self.data = sound.values
        self.sampling_frequency = sound.sampling_frequency

    def to_parselmouth_sound(self) -> "_PickableSound":
        """Convert the _PickableSound object back to a parselmouth.Sound object.

        Returns:
            parselmouth.Sound: The parselmouth.Sound object.
        """
        # Convert back to a parselmouth.Sound object
        return parselmouth.Sound(values=self.data, sampling_frequency=self.sampling_frequency)

def get_pickablesound(audio: Union[_PickableSound,
                                   parselmouth.Sound,
                                   Path,
                                   Audio], sampling_rate: int = 16000) -> _PickableSound:
    """Get a sound object from a given audio file or Audio object.

    Args:
        audio (Union[_PickableSound, parselmouth.Sound, Path, Audio]): A path to an audio file or an Audio object.
        sampling_rate (int, optional): The sampling rate of the audio. Defaults to 16000.

    Returns:
        _PickableSound: A wrapper to the Parselmouth Sound object.
    ...
    try:
        # Loading the sound
        if isinstance(audio, parselmouth.Sound):
            snd_full = audio
        elif isinstance(audio, _PickableSound):
            snd_full = audio.to_parselmouth_sound()
    ...
    return _PickableSound(snd_full)


def extract_speech_rate(snd: Union[_PickableSound, parselmouth.Sound, Path, Audio]) -> Dict[str, float]:
    """Extract speech timing and pausing features from a given sound object.

    Args:
        snd (Union[_PickableSound, parselmouth.Sound, Path, Audio]): A Parselmouth Sound object
            or a file path or an Audio object.
    ...
    try:
        ...
        if not isinstance(snd, _PickableSound):
            snd = get_pickablesound(snd)
        snd = snd.to_parselmouth_sound()
...
def extract_pitch_values(snd: Union[_PickableSound, parselmouth.Sound, Path, Audio]) -> Dict[str, float]:
...
def extract_pitch_descriptors(
    snd: Union[_PickableSound, parselmouth.Sound, Path, Audio],
    floor: float,
    ceiling: float,
...
def extract_intensity_descriptors(
    snd: Union[_PickableSound, parselmouth.Sound, Path, Audio], floor: float, frame_shift: float
) -> Dict[str, float]:
...
def extract_slope_tilt(snd: Union[_PickableSound,
                                  parselmouth.Sound,
                                  Path,
                                  Audio], floor: float, ceiling: float) -> Dict[str, float]:
...
def extract_cpp_descriptors(
    snd: Union[_PickableSound, parselmouth.Sound, Path, Audio], floor: float, ceiling: float, frame_shift: float
) -> Dict[str, float]:
...
def extract_audio_duration(snd: Union[_PickableSound, parselmouth.Sound, Path, Audio]) -> Dict[str, float]:
...
def extract_jitter(snd: Union[_PickableSound,
                              parselmouth.Sound,
                              Path,
                              Audio], floor: float, ceiling: float) -> Dict[str, float]:
    ...
    Args:
        snd (Union[_PickableSound, parselmouth.Sound, Path, Audio]): A Parselmouth Sound object, a file path (Path),
...
def extract_shimmer(snd: Union[_PickableSound,
                               parselmouth.Sound,
                               Path,
                               Audio], floor: float, ceiling: float) -> Dict[str, float]:
...
def extract_praat_parselmouth_features_from_audios(
    ...
    get_pickablesound_pt = pydra.mark.task(get_pickablesound)
    ...
    wf.split("x", x=audios)
    wf.add(get_pickablesound_pt(name="get_pickablesound_pt",
                                audio=wf.lzin.x))
    wf.add(extract_pitch_values_pt(name="extract_pitch_values_pt",
                                   snd=wf.get_pickablesound_pt.lzout.out))
    wf.add(
        _extract_pitch_floor_pt(name="_extract_pitch_floor_pt",
                                pitch_values_out=wf.extract_pitch_values_pt.lzout.out)
    )
    wf.add(
        _extract_pitch_ceiling_pt(
            name="_extract_pitch_ceiling_pt",
            pitch_values_out=wf.extract_pitch_values_pt.lzout.out
        )
    )
    if speech_rate:
        wf.add(extract_speech_rate_pt(name="extract_speech_rate_pt",
                                      snd=wf.get_pickablesound_pt.lzout.out))
    if pitch:
        wf.add(
    ...
    if duration:
        wf.add(extract_audio_duration_pt(name="extract_audio_duration_pt",
                                         snd=wf.get_pickablesound_pt.lzout.out))
    if jitter:
        wf.add(
            extract_jitter_pt(
    ...
    output_connections = [
        # ("pickablesound_out", wf.get_pickablesound_pt.lzout.out),
        # ("pitch_values_out", wf.extract_pitch_values_pt.lzout.out)
        ]

    if speech_rate:
        output_connections.append(("speech_rate_out",
                                   wf.extract_speech_rate_pt.lzout.out))
    if pitch:
        output_connections.append(("pitch_out",
                                   wf.extract_pitch_descriptors_pt.lzout.out))
    if intensity_descriptors:
        output_connections.append(("intensity_out",
                                   wf.extract_intensity_descriptors_pt.lzout.out))
    if harmonicity_descriptors:
        output_connections.append(("harmonicity_out",
                                   wf.extract_harmonicity_descriptors_pt.lzout.out))
    if formants:
        output_connections.append(("formants_out",
                                   wf.measure_f1f2_formants_bandwidths_pt.lzout.out))
    if spectral_moments:
        output_connections.append(("spectral_moments_out",
                                   wf.extract_spectral_moments_pt.lzout.out))
    if slope_tilt:
        output_connections.append(("slope_tilt_out",
                                   wf.extract_slope_tilt_pt.lzout.out))
    if cpp_descriptors:
        output_connections.append(("cpp_out",
                                   wf.extract_cpp_descriptors_pt.lzout.out))
    if duration:
        output_connections.append(("audio_duration",
                                   wf.extract_audio_duration_pt.lzout.out))
    if jitter:
        output_connections.append(("jitter_out",
                                   wf.extract_jitter_pt.lzout.out))
    if shimmer:
        output_connections.append(("shimmer_out",
                                   wf.extract_shimmer_pt.lzout.out))
