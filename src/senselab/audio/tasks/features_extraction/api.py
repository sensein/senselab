...
def extract_features_from_audios(audios: List[Audio], 
                                 plugin: str = "cf",
    ...
    if opensmile:
        ...
        wf.add(
            extract_opensmile_features_from_audios_pt(name="extract_opensmile_features_from_audios_pt", 
                                                    audios=wf.lzin.x,
    ...
    if torchaudio:
        default_torchaudio = {"freq_low": 80, 
                              "freq_high": 500, 
                              "n_fft": 1024, 
                              "n_mels": 128, 
                              "n_mfcc": 40, 
        ...
        if isinstance(torchaudio, dict):
            ...
            if "n_fft" in torchaudio:
                my_torchaudio['n_fft'] = (
                    int(torchaudio['n_fft']) 
                    if torchaudio['n_fft'] is not None 
                    else default_torchaudio['n_fft']
                )
            if "n_mels" in torchaudio:
                my_torchaudio['n_mels'] = (
                    int(torchaudio['n_mels']) 
                    if torchaudio['n_mels'] is not None 
                    else default_torchaudio['n_mels']
                )
            if "n_mfcc" in torchaudio:
                my_torchaudio['n_mfcc'] = (
                    int(torchaudio['n_mfcc']) 
                    if torchaudio['n_mfcc'] is not None 
                    else default_torchaudio['n_mfcc']
                )
        ...
        wf.add(
            extract_torchaudio_features_from_audios_pt(name="extract_torchaudio_features_from_audios_pt", 
                                                       audios=wf.lzin.x,
    ...
    if parselmouth:
        output_connections.append(("praat_parselmouth_out", 
                                   wf.extract_praat_parselmouth_features_from_audios_pt.lzout.out))
    if torchaudio:
        output_connections.append(("torchaudio_out", wf.extract_torchaudio_features_from_audios_pt.lzout.out))
    if torchaudio_squim:
        output_connections.append(("torchaudio_squim_out", 
                                   wf.extract_objective_quality_features_from_audios_pt.lzout.out))
