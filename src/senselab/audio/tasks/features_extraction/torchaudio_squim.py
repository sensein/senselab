...
def extract_subjective_quality_features_from_audios(audio_list: List[Audio], 
                                        non_matching_references: List[Audio]
                                        ) -> Dict[str, Any]:
    ...
    if any(audio.waveform.shape[0] != 1 for audio in audio_list) or \
       any(ref.waveform.shape[0] != 1 for ref in non_matching_references):
        raise ValueError("Only mono audio is supported by Torchaudio-Squim model.")

    # Check if any audio has a sampling rate other than 16000 Hz
    if any(audio.sampling_rate != 16000 for audio in audio_list) or \
       any(ref.sampling_rate != 16000 for ref in non_matching_references):
        raise ValueError("Only 16000 Hz sampling rate is supported by Torchaudio-Squim model.")
