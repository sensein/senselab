...
generated by ``scripts/build_classifier_ontology_map.py`` and is never hand-edited.

A HeAR label is corroborated by its mapped AudioSet node **or any descendant of that node**, so the
sets here are subtree closures. Every name in a set is an AudioSet ``display_name``, which is what
YAMNet and AST both report. :func:`corroboration_sets` answers whether an AudioSet name falls in a
HeAR label's subtree; :func:`canonical_names` answers which ontology node a label spelling denotes,
which is the mapped root alone. The airway kind is the closure of the ontology roots named in
``taxonomy.airway_ontology_roots``, and both its AudioSet and its HeAR evidence labels are read off
that one closure.
...
def load_classifier_ontology(path: str | None = None) -> dict[str, Any]:
    ...
    Raises:
        FileNotFoundError: If ``path`` names a file that does not exist; a named profile is never
...
def canonical_names(path: str | None = None) -> dict[str, str]:
    ...
    An AudioSet display name maps to itself; a HeAR label resolving onto one AudioSet node maps to
    that node's display name; a HeAR label resolving onto several is absent from the map.
...
def audioset_labels_under_roots(roots: Sequence[str], path: str | None = None) -> tuple[str, ...]:
    ...
    A node no classifier has an output for — see :func:`_emittable` — is left out.
