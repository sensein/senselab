...
from senselab.audio.workflows.triage.routing_analysis.families import DECLARED_KIND
...
class Confusion:
    ...
    def as_json(self) -> dict[str, Any]:
        ...
        Returns:
            The four counts plus sensitivity, specificity and Youden's J.
        """
        return {
            "tp": self.tp,
            "fp": self.fp,
...
def score_detector(
    ...
    *,
    ...
    """One detector's 2x2 table at every threshold in its grid.
    ...
    Returns:
        ...
        because the evidence was absent, and one row per threshold.
    """
    ...
    for record in records:
        ...
        if value is None:
            unavailable += 1
            continue
        values.append((value, reference.predicate(record)))
    rows = []
    for threshold in sweep_points(detector):
        ...
        for value, positive in values:
            ...
            if fired and positive:
                ...
                tn += 1
        row: dict[str, Any] = {"threshold": threshold, **Confusion(tp, fp, tn, fn).as_json()}
    ...
    return {
        ...
        "n_reference_positive": sum(1 for _, positive in values if positive),
        "rows": rows,
    }
    ...
    for scored in sweeps:
        ...
        for row in scored["rows"]:
            lines.append(
                ...
                f"{_rate(row['sensitivity'])} | {_rate(row['specificity'])} | {row.get('marker', '')} |"
            )
    lines.append("")
