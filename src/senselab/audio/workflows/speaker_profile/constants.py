...
#
# A profile is only scorable later for models whose per-window embeddings the
# consumer also extracts; that default set is ECAPA + ResNet. Keeping the build
# default identical means a default-built profile is fully consumed — no
# enrolled-but-unscored centroid, which is the asymmetry observed on the peds run.
#
# Any model reachable through ``extract_speaker_embeddings_from_audios`` can be
# passed explicitly instead; the two below are simply the defaults that are known
# to be scored downstream.
# ---------------------------------------------------------------------------

ECAPA_MODEL_ID: str = "speechbrain/spkrec-ecapa-voxceleb"
"""[reuse] SpeechBrain ECAPA-TDNN — default embedding model."""

RESNET_MODEL_ID: str = "speechbrain/spkrec-resnet-voxceleb"
"""[reuse] SpeechBrain ResNet-TDNN — second default."""

DEFAULT_EMBEDDING_MODELS: tuple[str, ...] = (
    ECAPA_MODEL_ID,
    RESNET_MODEL_ID,
)
"""Default embedding consensus set — ECAPA + ResNet, matching the per-window models
extracted downstream so a default-built profile is fully scorable. Two models give
some error decorrelation; a single-model fallback (e.g. just ECAPA) MUST remain
viable."""
