...
def read_files_from_disk(files: Union[str, List[str]]) -> Dict[str, Any]:
    ...
    def _from_files_to_dataset(files: List[File]) -> Dataset:
        """Reading files from disk and create a Hugging Face `Dataset` object."""
...
def read_dataset_from_disk(input_path: str, split: str, streaming: bool = False) -> Dict[str, Any]:
    """Loads a Hugging Face `Dataset` object from disk, determining the format based on the file extension or directory."""
    # Determine the input format based on the file extension or if it is a directory
    ...
    # Load the dataset
    try:
        dataset = load_dataset(input_format, split=split, data_files=input_path, streaming=streaming)
        return _from_hf_dataset_to_dict(dataset)
...
def read_dataset_from_hub(remote_repository: str, revision: str = "main", split: str = "all", hf_token: Optional[str] = None) -> Dict[str, Any]:
    """Loads a Hugging Face `Dataset` object from the Hugging Face Hub, including support for private repositories."""    
    if not _check_hf_repo_exists(remote_repository, "dataset", hf_token):
        raise RuntimeError(f"The repository {remote_repository} - {revision} - {split} does not exist or could not be accessed.")

    # Load the dataset
    try:
        dataset = load_dataset(path=remote_repository, revision=revision, split=split, token=hf_token)
    except Exception as e:
        # Generic error handling, e.g., network issues, data loading issues
        raise RuntimeError(f"An error occurred while loading the dataset: {str(e)}")
...
def push_dataset_to_hub(dataset: Dict[str, Any], remote_repository: str, revision: str = "main", split: str = "all", hf_token: Optional[str] = None) -> None:
    ...
    try:
        ...
        if hf_token:
            hf_dataset.push_to_hub(repo_id=remote_repository, revision=revision, split=split, private=True, token=hf_token)
        else:
...
def save_dataset_to_disk(dataset: Dict[str, Any], output_directory: str, output_name: str, output_format: str = "parquet") -> None:
    """Saves a Hugging Face `Dataset` object to disk."""
    # TODO: optimize saving process (e.g., playing with batch size and num of processors)
    hf_dataset = _from_dict_to_hf_dataset(dataset)

    # Prepare the output path differently if the format is Arrow
    if output_format == "arrow":
        output_path = os.path.join(output_directory, output_name)  # No extension for Arrow, it's a directory
    else:
...
def _from_dict_to_hf_dataset(data: Dict[str, Any], image_columns: list[str] = ['image'], audio_columns: list[str] = ['audio']) -> Dataset:
    """Converts a dictionary to a Hugging Face `Dataset` object with casting for image and audio columns."""
