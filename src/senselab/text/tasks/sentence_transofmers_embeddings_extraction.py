...
def extract_embeddings_from_hf_dataset(dataset: Dict[str, Any], model_id: str, model_revision: str = 'main', text_column: str = 'text') -> Dict[str, Any]:
    """Extracts embeddings from a Hugging Face `Dataset` object."""
    def _extract_embeddings_from_hf_row(row: Dataset, text_column: str, model: SentenceTransformer) -> Dataset:
    ...
    embeddings_hf_dataset = hf_dataset.map(lambda x: _extract_embeddings_from_hf_row(x, text_column, model))
