...
|----------|--------------|-------------|-----|----------|--------|-------|
| `audio.tasks.classification.classify_audios` | transformers | transformers>=4.40,<5.0 | Yes | No | >=3.11,<3.13 | >=2.8,<3.0 |
| `audio.tasks.features_extraction.extract_ppg_from_audios` | ppgs, espnet | ppgs>=0.0.9,<0.0.10, espnet>=202205, torch>=2.0,<2.9 | Yes | Yes (ppgs) | >=3.11,<3.13 | >=2.8,<3.0 |
| `audio.tasks.forced_alignment.align_transcriptions` | transformers, torchaudio | transformers>=4.40,<5.0, torchaudio>=2.8,<3.0 | Yes | No | >=3.11,<3.13 | >=2.8,<3.0 |
| `audio.tasks.speaker_diarization.diarize_audios` | pyannote-audio, torchaudio | pyannote-audio>=3.0,<5.0, torchaudio>=2.8,<3.0 | Yes | No | >=3.11,<3.13 | >=2.8,<3.0 |
| `audio.tasks.speaker_embeddings.extract_speaker_embeddings_from_audios` | speechbrain, torchaudio | speechbrain>=1.0,<2.0, torchaudio>=2.8,<3.0 | Yes | No | >=3.11,<3.13 | >=2.8,<3.0 |
| `audio.tasks.speech_enhancement.enhance_audios` | speechbrain, torchaudio | speechbrain>=1.0,<2.0, torchaudio>=2.8,<3.0 | Yes | No | >=3.11,<3.13 | >=2.8,<3.0 |
| `audio.tasks.speech_to_text.transcribe_audios` | transformers, torchaudio | transformers>=4.40,<5.0, torchaudio>=2.8,<3.0 | No | No | >=3.11,<3.13 | >=2.8,<3.0 |
| `audio.tasks.text_to_speech.synthesize_texts` | transformers | transformers>=4.40,<5.0 | Yes | No | >=3.11,<3.13 | >=2.8,<3.0 |
| `audio.tasks.voice_cloning.clone_voices` | coqui-tts | coqui-tts>=0.27,<1.0, torch>=2.4,<2.9 | Yes | Yes (coqui) | >=3.11,<3.13 | >=2.8,<3.0 |
| `text.tasks.embeddings_extraction.extract_embeddings_from_text` | transformers | transformers>=4.40,<5.0 | Yes | No | >=3.11,<3.13 | >=2.8,<3.0 |
| `video.tasks.pose_estimation.estimate_pose` | ultralytics, opencv-python-headless | ultralytics>=8.0,<9.0, opencv-python-headless>=4.8,<5.0 | No | No | >=3.11,<3.13 | >=2.8,<3.0 |
